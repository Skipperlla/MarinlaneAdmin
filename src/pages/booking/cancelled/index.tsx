...
import { Cancelleds } from "store/actions/bookingAction";
...
const Customers = () => {
  ...
  useEffect(() => {
    if (router.isReady) dispatch(Cancelleds(router.query));
        ...
        {loading ? (
          ...
          <div
            onClick={(e) => {
              ...
              if (Bookings?.count > 0) {
                api()
                  .get("/Admin/Booking/exportCancelledExcel", {
                    responseType: "blob",
                  })
                  .then((data) => {
                    fileDownload(data.data, "bookings_cancelled.xlsx");
                    setIsLoading(false);
                  })
